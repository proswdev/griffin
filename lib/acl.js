...
function Acl(name, actions) {
  ...
  function minimize(acl) {
    ...
    if (acl.acls && !acl.minimized) {
      if (acl.maximized)
        access._throw('Acl is either required or granted but not both');
      optimize(acl,false);
      acl.minimized = true;
    }
  }
  ...
  this.toString = function(options) {
    options = options || { roles: true, permissions: true, brackets: true, export: false };
    if (options.roles === undefined)
      options.roles = true;
    if (options.permissions === undefined)
      options.permissions = true;
    if (options.brackets === undefined)
      options.brackets = true;
    var items = [];
    if (this.mask) {
      // Convert mask to array string of corresponding acid names
      this.mask.forEach(function(acid) {
        var name = access._getAcidName(acid);
        var isRole = access.isRole(name);
        if ((options.roles && isRole) || (options.permissions && !isRole))
          items.push(name);
      });
      items = options.export ? items.join('.') : options.brackets ? '[' + items + ']' : items.toString();
    } else if (this.acls) {
      // Convert ACLs to combined list of mask strings
      this.acls.forEach(function(acl) {
        items.push(acl.toString(options));
      });
      if (items.length > 1)
        items = items.join(options.export ? '.or.' : ' || ');
    }
  ...
  this.extendTo = function() {
    // Optimize this instance for use as granted ACL
    maximize(this);
    var acl = options.getAcl.apply(this, arguments);
    if (acl)
      // Extend existing granted ACL with this instance
      acl.add(this);
    else
      // Delegate ACL assignment to framework specific arguments to external option method
      options.setAcl.apply(this, arguments);
    ...
    return (function() {
      ...
      return function() {
        ...
        if (allowed) {
          ...
          return __fnc.apply(__acl, arguments);
        } else {
          // Delegate framework specific rejection to external option method
          return options.reject.apply(__acl, arguments);
        }
      };
    })();
  };
    ...
    return {
      get: function() {
        ...
        return function() {
          ...
          var allowed = __acl.isGrantedTo.apply(__acl, arguments);
          if (allowed)
            return options.cont.apply(__acl, arguments);
          else
            return options.halt.apply(__acl, arguments);
        };
      }
    }
  })());
  ...
  if (typeof(name) === 'number' && typeof(actions) === 'number') {
    ...
    if (!access.isResource(name))
      access._throw("'" + name + "' is not a valid resource");
    self.mask = new AclMask();
    access._makeArray(actions).forEach(function(action) {
      var acid = access._getAcid(name, action);
      if (acid === undefined)
        access._throw("Action '" + action + "' not defined for resource '" + name + "'");
      self.mask.set(acid);
    });
    ...
    access._throw("'" + name + "' is not a valid role");
  }
}
