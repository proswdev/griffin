    ...
    function select(nextToken) {
        ...
        if (!_builder && _lastToken !== nextToken) {
            ...
            if (nextToken === 'except' && _lastToken !== 'role')
                access._throw("'except' applies to roles only", 500);
            var next = false;
            switch (_lastToken) {
                case 'resource':
                    // Resource tokens need always be followed by action token(s)
                    if (nextToken !== 'action')
                        access._throw('Resource(s) specified without action(s)', 500);
                    break;
                case 'action':
                ...
                default:
                    ...
                    if (nextToken === 'action')
                        access._throw('Action(s) specified without resource', 500);
