...
var _access = {
  ...
  getAcl: function(req) {
    var acl;
    if (req) {
      // Retrieve ACL granted to request
      acl = access.getAcl(req);
    } else if (_aclBuilder) {
      // Build and return ACL currently under construction
      acl = _aclBuilder.build();
      _aclBuilder = null;
    }
    return acl;
  },
  ...
  eval: function(aclstr) {
    ...
    aclstr.split('.').forEach(function(item){
      if (Object.getOwnPropertyDescriptor(_access, item))
        ...
        dummy = access._throw("'" + item + "' not defined");
    });
    return _access;
  },
  ...
  extendTo: function() {
    ...
    // with full unaltered argument list
    var acl = _access.getAcl();
    acl.extendTo.apply(acl, arguments);
  },
  ...
  filter: function() {
    // Complete current ACL and pass on 'filter' operation
    // with full unaltered argument list
    var acl = _access.getAcl();
    return acl.filter.apply(acl, arguments);
  },

  toString: function() {
    // Complete current ACL and pass on 'toString' operation
    // with full unaltered argument list
    var acl = _access.getAcl();
    return acl.toString.apply(acl, arguments);
  }
};
...
var access = {
  ...
  getAcl: function(req) {
    return Acl.options.getAcl(req);
  },
  isValid: function(aclstr) {
    var valid;
    try {
      access.eval(aclstr);
      valid = true;
  ...
  /*
  spec = {
    field1: { read/write/rdwr: string/acl, reject(opt): false/true },
    field2: { read/write/rdwr: string/acl, reject(opt): false/true }
  }
  data = obj.$extractFor(req/acl)
  obj.$updateBy(req/acl, data)

   */
  protect: function(specs, obj) {
    // Protect read/write of object properties
    var read = {};
    var write = {};
    for (var name in specs) {
      var spec = specs[name];
      if (spec.read) {
        if (typeof spec.read === 'string')
          spec.read = access.eval(spec.read).getAcl();
        else if (!(spec.read instanceof Acl))
          access._throw('invalid access specs');
        read[name] = spec.read;
      }
      if (spec.write) {
        if (typeof spec.write === 'string')
          spec.write = access.eval(spec.write).getAcl();
        else if (!(spec.write instanceof Acl))
          access._throw('invalid access specs');
        write[name] = spec.write;
      }
      if (spec.rdwr) {
        if (typeof spec.rdwr === 'string')
          spec.rdwr = access.eval(spec.rdwr).getAcl();
        else if (!(spec.rdwr instanceof Acl))
          access._throw('invalid access specs');
        read[name] = write[name] = spec.rdwr;
      }
    }
    obj = obj || {};
    obj.$extractFor = (function() {
      var __read = read;
      return function(req, obj) {
        obj = obj || this;
        var out = {};
        for (var field in obj) {
          if (!__read[field] || __read[field].isGrantedTo(req))
            out[field] = obj[field];
        }
        return out;
      }
    })();
    obj.$updateBy = (function() {
      var __write = write;
      return function(req, updates, obj) {
        obj = obj || this;
        for (var field in updates) {
          if (obj[field] === undefined || obj[field] !== updates[field])
            if (!__write[field] || __write[field].isGrantedTo(req))
...
function _validate(name) {
  ...
  if (_reserved.indexOf(name) >= 0)
    access._throw("'" + name + "' is a reserved word");
}
...
function _definePermission(name, actions) {
  ...
  if (!name)
    access._throw('no resource name specified');
  _validate(name);
  if (access.isRole(name))
    access._throw("'" + name + "' already defined as role");
  ...
  if (!actions)
    access._throw("Resource '" + name + "' specified without any actions");
...
function _defineRole(name, accessDef) {
  ...
  // Validate role name
  if (!name)
    access._throw('no role name specified');
  _validate(name);
  if (access.isResource(name))
    access._throw("'" + name + "' already defined as resource");
